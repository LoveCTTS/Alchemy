...
class _FriendsChatPageState extends State<FriendsChatPage> {
  ...
  _sendMessage() {
    if (messageEditingController.text.isNotEmpty) { //메세지 입력란에 text가 비어있지않다면
      ...
      DatabaseService().sendMessageInFriendChat(widget.groupId, chatMessageMap); //특정 groupId에 위 Map<String,dynamic>형태의 데이터 전송
  ...
  void initState() {
    ...
    DatabaseService().getFriendChats(widget.groupId).then((val) {
      // print(val);
      setState(() {
        _chats = val;
      });
